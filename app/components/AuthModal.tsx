...
export default function AuthModal({ onClose, onAuthenticated }: { onClose: () => void; onAuthenticated: (user: DummyUser) => void }) {
  ...
  const submit = async (event: FormEvent) => { event.preventDefault(); const normalizedEmail = email.trim().toLowerCase(); if (!normalizedEmail.endsWith("@gmail.com")) { setError("Gunakan email Gmail yang berakhiran @gmail.com."); return; } setLoading(true); setError(""); try {
    ...
    if (result.error) { const offline = /failed to fetch|network|fetch|email not confirmed|rate limit|too many/i.test(result.error.message); if (offline && useFallback()) return; setError(result.error.message.includes("Invalid login") ? "Email atau password salah." : result.error.message); return; }
    if (mode === "signup" && !result.data.session) { const fallback = signUpDummy(normalizedEmail, password); if (fallback) { onAuthenticated(fallback); return; } setError("Akun berhasil dibuat. Silakan konfirmasi email lalu masuk."); return; }
