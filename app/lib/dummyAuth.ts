...
export function getDummySession(): DummyUser | null { if (typeof window === "undefined") return null; const value = window.localStorage.getItem(SESSION_KEY); return value ? JSON.parse(value) as DummyUser : null; }
export function signOutDummy() { window.localStorage.removeItem(SESSION_KEY); window.dispatchEvent(new Event("rumah-rona-auth")); }
