...
export function signUpDummy(email: string, password: string): DummyUser | null { const key = email.trim().toLowerCase(); if (!key || !key.endsWith("@gmail.com") || key === ADMIN_EMAIL || password.length < 6) return null; const users = getLocalUsers(); users[key] = password; window.localStorage.setItem(USERS_KEY, JSON.stringify(users)); const user = { email: key, role: "customer" as const }; window.localStorage.setItem(SESSION_KEY, JSON.stringify(user)); return user; }
