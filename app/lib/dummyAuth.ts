...
export function getDummySession(): DummyUser | null { if (typeof window === "undefined") return null; const value = window.localStorage.getItem(SESSION_KEY); if (!value) return null; const user = JSON.parse(value) as DummyUser; return user.role === "admin" && window.location.pathname === "/" ? null : user; }
