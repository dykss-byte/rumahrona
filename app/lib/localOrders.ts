...
export function getSavedOrderStatus(orderNumber: string): OrderStatus | null { if (typeof window === "undefined") return null; const statuses = JSON.parse(window.localStorage.getItem(STATUS_KEY) ?? "{}") as Record<string, OrderStatus>; return statuses[orderNumber] ?? null; }
